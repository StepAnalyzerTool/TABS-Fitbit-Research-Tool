...
def render_activity_alert_test(steps_df):
    with st.expander("🚶 Activity Alert Test", expanded=False):
        ...
        if st.button("Evaluate step-alert rules now",type="primary"):
            ...
            if new:
                milestone=max(new); text=f"🎉 Good job! You reached another 500 steps! Total for today is {total_steps:,} steps."; keyboard={"inline_keyboard":[[{"text":"👍 Got it","callback_data":f"step_got_it:{milestone}:{int(time.time())}"}]]}
                result=post("sendMessage",{"chat_id":chat_id,"text":text,"reply_markup":keyboard}); st.session_state.step_milestones_sent.update(milestones); st.session_state.activity_alert_log.append({"type":"500-step milestone","criterion":milestone,"criterion_time":"available at check","sent_time":now.strftime("%Y-%m-%d %H:%M:%S"),"message_id":result.get("message_id"),"response":"","response_time":"","latency":""}); st.success(f"Sent milestone alert. Current total: {total_steps:,} steps.")
        ...
        if st.button("Check activity-alert responses"):
            try:
                ...
                for update in get_updates():
                    cb=update.get("callback_query") or {}; cid=str(cb.get("id",""))
                    if not cid or cid in st.session_state.processed_alert_callbacks: continue
                    msg=cb.get("message") or {}; mid=msg.get("message_id"); data=cb.get("data","")
                    if not (data.startswith("step_got_it:") or data.startswith("on_it:") or data.startswith("remind30:")): continue
                    response_time=datetime.now(); matching=next((row for row in reversed(st.session_state.activity_alert_log) if row.get("message_id")==mid),None)
                    if matching:
                        sent=datetime.strptime(matching["sent_time"],"%Y-%m-%d %H:%M:%S"); seconds=max(0,int((response_time-sent).total_seconds())); matching["response"]={"step_got_it":"Got it","on_it":"On it!","remind30":"Remind me later"}.get(data.split(":")[0],data); matching["response_time"]=response_time.strftime("%Y-%m-%d %H:%M:%S"); matching["latency"]=f"{seconds//60}m {seconds%60}s"
                        post("answerCallbackQuery",{"callback_query_id":cid}); post("editMessageReplyMarkup",{"chat_id":chat_id,"message_id":mid,"reply_markup":{"inline_keyboard":[]}}); found+=1
                    st.session_state.processed_alert_callbacks.add(cid)
                if found: st.success(f"Recorded {found} response(s) and removed the used Telegram button(s).")
