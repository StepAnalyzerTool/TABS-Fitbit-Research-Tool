...
def render_activity_alert_test(steps_df):
    with st.expander("🚶 Activity Alert Test", expanded=False):
        ...
        now=now_eastern();today=now.date();day=steps_df[steps_df["minute"].dt.date==today].copy();total_steps=int(day["steps_per_minute"].fillna(0).sum()) if not day.empty else 0
        st.metric("Steps currently available for today",total_steps);st.caption(f"Current app time: {now.strftime('%I:%M:%S %p %Z')}")
        ...
        if st.session_state.activity_alert_log:st.subheader("Alert log");st.dataframe(st.session_state.activity_alert_log,use_container_width=True,hide_index=True)
